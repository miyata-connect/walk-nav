...
// WalkNav app.js - v85: 全機能実装版 - フリーズ問題修正
// 未実装のイベントハンドラを全て追加

const ISSUE_ID = ‘idx20251217_v85_all_features_implemented’;

const API_KEY = ‘AIzaSyBuX-4y1Cgl6jdKcHZWWlsoosDWK_RGqF0’;
const WORKER_ORIGIN = ‘https://ors-proxy.miyata-connect-jp.workers.dev’;

// 【重要】地図が消えるのを防ぐため、Google公式のデモIDを使用
...
const WN = (window.**WN_GLOBAL** = window.**WN_GLOBAL** || {
booted: false,
...
try {
  ...
  refreshSavedMarkers();
  console.log('[WalkNav] Map initialized v85');
} catch (e) {
...
```
// ルートを再計算
if (appState.currentDestination && appState.currentRouteData) {
  const routes = appState.currentRouteData.routes;
  let chosen = { route: routes[0], index: 0 };
  if (window.RouteEvaluator?.pickBestRoute) {
...
// パネル内のボタン
if (getEl('btnPointSearch')) getEl('btnPointSearch').onclick = togglePointSearchMode;
if (getEl('btnReset')) getEl('btnReset').onclick = resetApp;
...
if (appState.pointSearchMode && appState.searchPoint) {
  lat = appState.searchPoint.lat;
  lng = appState.searchPoint.lng;
  addrText = getEl('pointAddress')?.textContent || '住所不明';
} else if (appState.currentPos) {
  lat = appState.currentPos.lat;
  lng = appState.currentPos.lng;
  addrText = getEl('locAddress')?.textContent || '現在地';
} else {
  return alert('現在地が取得できていません');
}
...
function startApp() {
console.log(’[WalkNav] Starting v85 (All Features Implemented)…’);
loadUserProfile();
loadSavedLocations();
bindUI();
renderSavedLocations();
