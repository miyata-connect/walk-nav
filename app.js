...
function startLocationWatcher() {
    ...
    appState.locationWatchId = navigator.geolocation.watchPosition(
        (pos) => {
            ...
            // ステップ進行チェック（音声案内）
            checkStepProgress(latitude, longitude);
...
async function startNavigation(destination) {
    ...
    try {
        ...
        if (result.routes && result.routes.length > 0) {
            ...
            // 音声案内: ステップデータ保存 + 開始アナウンス
            if (l0.steps) {
                appState.currentSteps = l0.steps;
                appState.currentStepIndex = 0;
            }
            speakTTS(destination.name + 'へのルートを案内します。' + readLegDistanceText(l0) + '、徒歩' + readLegDurationText(l0) + 'です。');
            if (l0.steps && l0.steps.length > 0) {
                setTimeout(() => {
                    const first = l0.steps[0].html_instructions.replace(/<[^>]+>/g, ' ');
                    speakTTS('まず、' + first);
                }, 4000);
            }
...
function bindUI() {
    ...
    const btnRouteAi = getEl('btnRouteAiShortest');

    if (btnPause) btnPause.onclick = togglePause;
    if (btnReroute) btnReroute.onclick = executeReroute;
    if (btnExport) btnExport.onclick = exportRouteToClipboard;
    if (btnRouteNormal) btnRouteNormal.onclick = () => setRouteMode('normal');
    if (btnRouteAi) btnRouteAi.onclick = () => setRouteMode('shortest');
...
function togglePause() {
    ...
    const btn = getEl('btnPauseSettings');
    if (btn) {
        btn.textContent = appState.isPaused ? '▶️ 再開' : '⏸️ 一時停止';
    }
    if (appState.isPaused) {
        stopLocationWatcher();
        stopTTS();
        speakTTS('ナビゲーションを一時停止しました');
    } else {
        if (!appState.isSimulation) startLocationWatcher();
        speakTTS('ナビゲーションを再開します');
    }
}
...
function executeReroute() {
    if (!appState.isNavigating || !appState.currentDestination) {
        console.warn('[WalkNav] リルート: 目的地が設定されていません');
        return;
    }
    speakTTS('ルートを再検索します');
...
function exportRouteToClipboard() {
    if (!appState.currentSteps || appState.currentSteps.length === 0) {
        alert('コピーする道順がありません');
        return;
    }
    const dest = appState.currentRouteData?.destinationName || '目的地';
    let text = dest + 'への道順:\n\n';
    appState.currentSteps.forEach((step, i) => {
        const instruction = step.html_instructions.replace(/<[^>]+>/g, ' ').trim();
        const dist = step.distance?.text || '';
        text += (i + 1) + '. ' + instruction + ' (' + dist + ')\n';
    });
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => {
            alert('道順をクリップボードにコピーしました');
        }).catch(() => {
            fallbackCopy(text);
        });
    } else {
...
function setRouteMode(mode) {
    ...
    const btnNormal = getEl('btnRouteNormal');
    const btnAi = getEl('btnRouteAiShortest');
    if (btnNormal) btnNormal.classList.toggle('active', mode === 'normal');
    if (btnAi) btnAi.classList.toggle('active', mode === 'shortest');
    if (appState.isNavigating && appState.currentDestination) {
        speakTTS(mode === 'shortest' ? '最短ルートに切り替えます' : '標準ルートに切り替えます');
        startNavigation(appState.currentDestination);
    }
}

// ===== ステップ進行トラッキング（位置更新時に呼ばれる） =====
function checkStepProgress(lat, lng) {
    if (!appState.isNavigating || appState.isPaused) return;
    if (!appState.currentSteps || appState.currentSteps.length === 0) return;

    const idx = appState.currentStepIndex;
    if (idx >= appState.currentSteps.length) return;

    const step = appState.currentSteps[idx];
    if (!step.end_location) return;

    const dist = calculateDistance(lat, lng, step.end_location.lat, step.end_location.lng);

    // 30m以内で次ステップに進む
    if (dist < 30) {
        appState.currentStepIndex = idx + 1;
        if (appState.currentStepIndex < appState.currentSteps.length) {
            const next = appState.currentSteps[appState.currentStepIndex];
            const instruction = next.html_instructions.replace(/<[^>]+>/g, ' ').trim();
            speakTTS(instruction);
        } else {
            // 最終ステップ完了 = 到着
            speakTTS('まもなく目的地に到着します');
        }
    }
}
