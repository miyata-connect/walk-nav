...
if (result.routes && result.routes.length > 0) {
  ...
  // MVP Phase 2: Route Evaluation & Sorting
  if (typeof RouteEvaluator !== 'undefined') {
    const wbgt = await RouteEvaluator.fetchWBGT();
    let safeRoutes = RouteEvaluator.filterSafeRoutes(result.routes);

    if (safeRoutes.length > 0) {
        // Calculate scores for ALL safe routes
        const scoredRoutes = safeRoutes.map((route, index) => {
            const score = RouteEvaluator.calculateStressScore(route, appState.userProfile, wbgt);
            return { route, score, originalIndex: index };
        });

        // Sort by Stress Score (Ascending: Lower is better)
        scoredRoutes.sort((a, b) => a.score - b.score);

        // Select the best route
        const bestCandidate = scoredRoutes[0];
        const bestRoute = bestCandidate.route;

        console.log(`[RouteEvaluator] WBGT: ${wbgt}. Best Score: ${bestCandidate.score}. Routes evaluated: ${scoredRoutes.length}`);

        // Update display to use the Best Route instead of default [0]
        // Note: r0 (default) was used for text display above. Ideally we update texts.
        const l0 = bestRoute.legs ? bestRoute.legs[0] : null;
        setText('routeDistance', readLegDistanceText(l0));
        setText('routeTime', `徒歩 ${readLegDurationText(l0)}`);

        // MVP Feedback UI
        const infoSection = getEl('routeInfoSection');
        let scoreEl = getEl('stressScoreDisplay');
        if (!scoreEl) {
            scoreEl = document.createElement('div');
            scoreEl.id = 'stressScoreDisplay';
            scoreEl.style.cssText = 'margin-top:8px;padding:8px;background:rgba(0,0,0,0.2);border-radius:8px;font-size:13px;';
            infoSection.appendChild(scoreEl);
        }
        scoreEl.innerHTML = `<div>🛡️ 安全性: OK</div><div>😓 ストレス値: <strong>${bestCandidate.score}</strong> (WBGT: ${wbgt})</div><div style="font-size:11px;opacity:0.7">推奨ルート表示中 (候補${scoredRoutes.length}件)</div>`;

        // Update Polyline to the Best Route
        drawRoutePolyline(bestRoute);

        // Use Best Route for instructions
        const list = getEl('navPanelInstructions');
        if (list && l0.steps) {
            list.innerHTML = '';
            l0.steps.forEach(step => {
        ...
        appState.currentRouteData = { destinationName: destination.name, summary: bestRoute.summary };

    } else {
        console.warn('[RouteEvaluator] No safe routes found.');
        alert('警告: 安全基準を満たすルートが見つかりませんでした。');
        // Fallback to default rendering if safety check fails everything (unlikely in mock)
        drawRoutePolyline(r0);
    }
  } else {
      // Fallback if Evaluator missing
      drawRoutePolyline(r0);
  }
...
const inputLuggage = getEl('userLuggage');
const inputCondition = getEl('userCondition');
const inputCompanion = getEl('userCompanion');

if (inputLuggage) inputLuggage.onchange = (e) => { appState.userProfile.luggage = e.target.value; console.log('Profile Updated:', appState.userProfile); };
if (inputCondition) inputCondition.onchange = (e) => { appState.userProfile.condition = e.target.value; console.log('Profile Updated:', appState.userProfile); };
if (inputCompanion) inputCompanion.onchange = (e) => { appState.userProfile.companion = e.target.value; console.log('Profile Updated:', appState.userProfile); };
