...
const ISSUE_ID = 'idx20251203_global_lock_v1';
...
function alertOnce(key, msg, ms = 900) {
  const now = Date.now();
...
if (WN.flags.appStarted) {
  console.warn('[WalkNav] app.js duplicate start blocked:', ISSUE_ID);
} else {
  WN.flags.appStarted = true;
  ...
  function setDisplay(id, v) { const el = getEl(id); if (el) el.style.display = v; }
  ...
  async function placesTextSearch(payload, fieldMask) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`TextSearch ${resp.status} ${body.slice(0, 160)}`);
    }
  ...
  function initMap(center) {
    ...
    try {
      ...
      alertOnce('map_fail', '地図の読み込みに失敗しました。APIキーの設定を確認してください。', 1200);
    }
  }
  ...
  function acquireLocation() {
    const onSuccess = (pos) => {
      ...
      else if (appState.map) appState.map.setCenter({ lat: latitude, lng: longitude });
  ...
  async function performSearch(query) {
    ...
    // ★二重発火しても「検索処理は1回だけ」
    if (!lockOnce('search_action', 800)) return;
    ...
    if (!center) {
      ...
      alertOnce('no_center', '現在地が取得できていません', 1200);
      return;
    }
  ...
  function displayResults(places) {
    ...
    places.forEach((p, i) => {
      ...
      item.addEventListener('click', () => {
        alertOnce('nav_not_implemented', 'このビルドではナビ開始部分が未結合です（元の1185行版に統合が必要）', 1200);
      });
  ...
  /* ===== 登録地ボタン（2回出る対策はwindowロックで吸収） ===== */
  function showSaveLocationDialog() {
    if (!lockOnce('save_location', 1500)) return;
  ...
  function showEditLocationDialog() {
    if (!lockOnce('edit_location', 900)) return;
    ...
    appState.savedLocations.forEach((loc, index) => {
      ...
      item.addEventListener('click', () => {
        safeRemove(overlay);
        appState.isEditDialogOpen = false;
        alertOnce('edit_menu_missing', '編集メニューは元の1185行版に統合が必要です', 1200);
      });
  ...
  function bindReliableActivate(el, fn, lockKey, lockMs) {
    if (!el) return;
    if (el.__wn_bound__) return;
    el.__wn_bound__ = true;

    const run = (e) => {
      if (lockKey && !lockOnce(lockKey, lockMs || 700)) return;
    ...
    else if ('ontouchend' in window) el.addEventListener('touchend', run, { passive: false });
  ...
  function bindUI() {
    ...
    bindReliableActivate(getEl('btnSearchIcon'), () => performSearch(inputQ ? inputQ.value : ''), 'tap_search', 650);
    ...
    bindReliableActivate(getEl('btnLocatePanel'), () => acquireLocation(), 'tap_locate1', 650);
    bindReliableActivate(getEl('btnLocate'), () => acquireLocation(), 'tap_locate2', 650);
    ...
    bindReliableActivate(getEl('btnSaveLocation'), () => showSaveLocationDialog(), 'tap_save', 1200);
    bindReliableActivate(getEl('btnEditLocation'), () => showEditLocationDialog(), 'tap_edit', 900);
    ...
    if (btnMapPhoto) bindReliableActivate(btnMapPhoto, () => changeMapMode('photo'), 'tap_map_photo', 650);
    if (btnMapRoadmap) bindReliableActivate(btnMapRoadmap, () => changeMapMode('roadmap'), 'tap_map_road', 650);
    if (btnMap3D) bindReliableActivate(btnMap3D, () => changeMapMode('3d'), 'tap_map_3d', 650);
