 ...
 * - appStateにuserProfileを統合
 * - bindUIにuserProfileの入力ハンドラを統合
 * - 登録地関連関数を二重発火対策後のイベントハンドラに修正
 */

// 【統合】メインブランチのISSUE_IDを継承し、マージバージョンを追記
const ISSUE_ID = 'idx20251207_merged_mvp_v2'; 
...
if (WN.booted) {
    ...
    // 【統合】Jules氏のMVP定義とメインブランチの安定化コードを統合
    const appState = {
        map: null,
        userMarker: null,
        currentPos: null,
        ...
        // 【統合】Jules氏のブランチからuserProfileを追加
        userProfile: {
            luggage: 'None',
            condition: 'Normal',
            companion: 'None'
        },
        // 【統合】メインブランチの検索ステータスとUI状態
        unifiedHeight: null,
        savedLocations: [],
...
     ========================= */
    ...
    /* =========================
     UI Fix: パネル高さ/スクロール/並び替え (メインブランチのロジックを採用)
     ========================= */
...
     ========================= */
    ...
    // 【統合】Jules氏のブランチから距離/時間のテキスト読み取りヘルパーを採用
...
     ========================= */
    async function startNavigation(destination) {
        ...
        try {
            ...
            // MVP: Route Evaluation (Jules氏のブランチから統合 - 後でRouteEvaluatorを定義する前提)
            if (typeof RouteEvaluator !== 'undefined') {
                const wbgt = await RouteEvaluator.fetchWBGT();
                const safeRoutes = RouteEvaluator.filterSafeRoutes(result.routes);

                if (safeRoutes.length > 0) {
                    const score = RouteEvaluator.calculateStressScore(safeRoutes[0], appState.userProfile, wbgt);
                    console.log(`[RouteEvaluator] WBGT: ${wbgt}, Score: ${score}, Profile:`, appState.userProfile);
                    ...
                    scoreEl.innerHTML = `<div>🛡️ 安全性: OK</div><div>😓 ストレス値: <strong>${score}</strong> (WBGT: ${wbgt})</div>`;
                } else {
                    console.warn('[RouteEvaluator] No safe routes found.');
                    alertOnce('no_safe_route', '警告: 安全基準を満たすルートが見つかりませんでした。');
                }
            }
...
     ========================= */
    ...
    function bindUI() {
        ...
        // 【統合】MVP: User Profile Inputs (Jules氏のブランチから統合)
        const inputLuggage = getEl('userLuggage');
        const inputCondition = getEl('userCondition');
        const inputCompanion = getEl('userCompanion');

        if (inputLuggage) inputLuggage.addEventListener('change', (e) => { appState.userProfile.luggage = e.target.value; console.log('Profile Updated:', appState.userProfile); });
        if (inputCondition) inputCondition.addEventListener('change', (e) => { appState.userProfile.condition = e.target.value; console.log('Profile Updated:', appState.userProfile); });
        if (inputCompanion) inputCompanion.addEventListener('change', (e) => { appState.userProfile.companion = e.target.value; console.log('Profile Updated:', appState.userProfile); });
        
        // 【統合】Jules氏のブランチのキーボードウォッチャーはUI Fixロジックと重複するため、ここでは除外
        // bindKeyboardWatch(); 
