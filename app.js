...
  // [!] この関数はGoogle Weather用です。
  // OpenWeatherのレスポンス形式に合わせて、ここも修正が必要です。
  // とりあえず、API呼び出しを優先して修正します。
...
      lat: lat,
      lon: lng, // 緯度は 'lon'
      units: 'metric' // 'METRIC' ではなく 'metric'
    };
...
    // [!] 警告: ここから下はGoogle Weather用のパーサーです。
    // worker.js が返す OpenWeather の形式 (data.weather[0].main など) に
    // 合わせて修正する必要があります。
    // (例: OpenWeatherは forecastHours を返さないため、ここはエラーになります)
