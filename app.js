  ...
  function initMap(center) {
    ...
    try {
      appState.map = new google.maps.Map(mapEl, {
        center,
        zoom: 17,
        // mapId: 'DEMO_MAP', // 地図が表示されない問題の対策として削除
...
<path d="M12 2L4.5 20.5L12 16.5L19.5 20.5L12 2Z" fill="#3aa0ff" stroke="#ffffff" stroke-width="2" stroke-linejoin="round" />\
  ...
  async function performSearch(query) {
    ...
    console.log('[WalkNav] Search centered at:', center.lat, center.lng, 'Radius:', appState.searchRadiusMeters);
  ...
  function showLocationDeleteConfirm(index) {
    ...
    btnGroup.appendChild(btnYes); // 閉じ忘れを修正
    dialog.appendChild(btnGroup); // btnGroupをdialogに追加
