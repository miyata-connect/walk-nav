...
if (window.__WALKNAV_APP_JS_STARTED__) {
  ...
  async function placesTextSearch(payload, fieldMask) {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`TextSearch ${resp.status}${detail ? `: ${detail.slice(0, 200)}` : ''}`);
    }
...
<path d="M12 2L4.5 20.5L12 16.5L19.5 20.5L12 2Z" fill="#3aa0ff" stroke="#ffffff" stroke-width="2" stroke-linejoin="round" />\
  ...
  async function startNavigation(destination) {
    ...
    if (appState.pointSearchMode && appState.searchPoint) {
      ...
      alertOnce('no_origin', '起点が取得できません', 1200);
      return;
    }
    ...
    try {
      ...
      if (!result.routes || result.routes.length === 0) {
        alertOnce('no_route', 'ルートが見つかりませんでした', 1200);
        stopNavigation();
        return;
      }
      ...
      console.error(e);
      alertOnce('route_fail', 'ルート検索エラー', 1200);
      stopNavigation();
    }
  }
  ...
  function showEditLocationDialog() {
    ...
    try {
      ...
      appState.isEditDialogOpen = false;
      console.error(e);
      alertOnce('edit_dialog_fail', '登録地編集ダイアログの表示に失敗しました', 1200);
    }
  }
  ...
  function showLocationEditMenu(index) {
    ...
    if (!location) {
      alertOnce('no_target', '対象の登録地が見つかりません', 1200);
      return;
    }
  ...
  function showLocationDeleteConfirm(index) {
    ...
    const location = appState.savedLocations[index];
    if (!location) {
      alertOnce('no_target_del', '対象の登録地が見つかりません', 1200);
      return;
    }
  ...
  function showLocationEditForm(index) {
    ...
    const location = appState.savedLocations[index];
    if (!location) {
      alertOnce('no_target_edit', '対象の登録地が見つかりません', 1200);
      return;
    }
    ...
    btnComplete.addEventListener('click', () => {
      ...
      if (!newName) {
        alertOnce('need_name', '登録地名を入力してください', 900);
        return;
      }
